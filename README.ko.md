...
| 플로팅 창에서 `Ctrl + S` | 현재 가로폭·아웃라인 설정을 적용해 설정한 저장 폴더에 JPG 저장 (기본: 바탕화면) |
...
현재 저장되는 주요 값은 클립보드 이미지 크기(Original Size 또는 가로 400~1600px), 복사/저장 이미지 아웃라인 여부, `Ctrl + S` 저장 폴더(기본: 바탕화면), 선택 텍스트 번역 언어, 번역 단축키, 이미지 번역 메뉴 언어, 매뉴얼/UI 언어, 번역 최초 1회 동의 여부입니다. 클립보드에는 호환성이 높은 Windows 비트맵으로 복사되며, 툴팁의 용량은 JPG 품질 85로 저장할 경우의 예상값입니다.
